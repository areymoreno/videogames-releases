...
function getGamesMonth (appendMonth, initialDate, finishDate, API_KEY) {
    ...
    let requestURL = 'https://api.rawg.io/api/games?key=' + API_KEY + '&dates=' + initialDate + ',' + finishDate + '&page_size=100&ordering=-released&platforms=1,186';
...
function apppendRelease (appendRelease, name, date, platform) {
    ...
    $(releaseAppend).append('<li class="list-group-item d-flex justify-content-between align-items-center">' +
        ...
        '<div class="text-end"><span id="badgeCSS" class="badge text-bg-success"><i class="bi bi-xbox"></i></span></div></li>');
